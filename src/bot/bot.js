...
function stopBot(userId) {
  if (!botState.sessions[userId]) {
    require('../utils/broadcast')({ type: 'log', message: `No active session for user ${userId}`, userId });
    return;
  }
  ...
  require('../utils/broadcast')({ type: 'log', message: `Bot stopped for user ${userId}`, userId });
  require('../utils/broadcast')({ type: 'status', userId, running: false });
}
...
function startBot(userId, cookieContent, prefix, adminID) {
  ...
  const tryLogin = (attempt = 1, maxAttempts = 1) => {
    ...
    if (attempt > maxAttempts) {
      ...
      botState.sessions[userId].api = null;
      require('../utils/broadcast')({ type: 'log', message: `Login failed – switched to SAFE MODE. Bot server alive, but no FB actions. Update cookies & restart!`, userId });
      return;
    }
    ...
    try {
      ...
      require('fca-mafiya').login(cookieContent, {}, (err, api) => {
        if (err || !api) {
          console.error(`[SAFE] Login failed for user ${userId} (attempt ${attempt}):`, err?.message || err);
          botState.sessions[userId].safeMode = true;
          botState.sessions[userId].running = true;
          botState.sessions[userId].api = null;
          require('../utils/broadcast')({ type: 'log', message: `Single login attempt failed. SAFE MODE activated to protect ID. Update cookies!`, userId });
          return;
        }
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 1) => {
          ...
          setInterval(() => {
            ...
            // Assuming cleanupMafiaGames is in mafia.js
            require('./mafia').cleanupMafiaGames(botState);
          }, 300000);
          ...
          api.listenMqtt(async (err, event) => {
            if (err) {
              ...
              botState.sessions[userId].safeMode = true;
              require('../utils/broadcast')({ type: 'log', message: `Connection lost – SAFE MODE activated. Bot alive, update cookies.`, userId });
              return;
            }
            ...
            try {
              ...
              if (event.type === 'message' || event.type === 'message_reply') {
                ...
                if (content.startsWith(botState.sessions[userId].prefix)) {
                  ...
                  const cmd = require('./commands').commands.get(command);
                  if (cmd) {
...
🛠️...use #help for commands...🛠️`, threadID, messageID);
              ...
              if (event.type === 'message') {
                ...
                if (lowerMsg.includes('bot') && isGroup) {
                  const randomResponse = require('../responses/randomBotReplies')[Math.floor(Math.random() * require('../responses/randomBotReplies').length)];
                  sendBotMessage(api, randomResponse, threadID, messageID);
                  responseSent = true;
                  return;
                }
              ...
              if (event.logMessageType === 'log:unsubscribe') {
                ...
                if (!leftID) {
                  require('../utils/broadcast')({ type: 'log', message: `Missing leftParticipantFbId in thread ${threadID}`, userId });
                  return;
                }
