...
const messageStore = require('./utils/messageStore'); // Added for message storage
...
function sendBotMessage(api, text, threadID, callback, mentions = []) {
  api.sendMessage({ body: text, mentions }, threadID, (err, messageInfo) => {
    if (!err && messageInfo?.messageID) {
      const replyToMessageID = callback && typeof callback === 'string' ? callback : null; // If callback is messageID (for replies)
      messageStore.storeBotMessage(messageInfo.messageID, text, threadID, replyToMessageID);
    }
      ...
      wiegine.login(cookieContent, {}, (err, api) => {
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 3) => {
          ...
          api.listenMqtt(async (err, event) => {
            ...
            // Skip read_receipt events to reduce clutter
            ...
            try {
              ...
              if (event.type === 'message') {
                ...
                messageStore.storeMessage(messageID, msg, senderID, threadID);

                // #delete on/off command
                if (msg.toLowerCase().startsWith('#delete') && (isMaster || botState.adminList.includes(senderID))) {
                  const action = msg.toLowerCase().split(' ')[1];
                  if (action === 'on') {
                    messageStore.deleteNotifyEnabled[threadID] = true;
                    sendBotMessage(api, '✅ डिलीट नोटिफिकेशन चालू कर दिया गया।', threadID);
                  } else if (action === 'off') {
                    messageStore.deleteNotifyEnabled[threadID] = false;
              ...
              if (event.type === 'message_unsend' && messageStore.deleteNotifyEnabled[threadID]) {
                  ...
                  if (deletedMsg) {
                    api.getUserInfo(deletedMsg.senderID, (err, info) => {
                      if (err || !info || !info[deletedMsg.senderID]) {
                        sendBotMessage(api, `@Unknown ने मैसेज डिलीट किया: "${deletedMsg.content}"`, threadID);
                        return;
                      }
                      const senderName = info[deletedMsg.senderID].name || 'Unknown';
                      sendBotMessage(api, `@${senderName} ने मैसेज डिलीट किया: "${deletedMsg.content}"`, threadID, null, [
                        { tag: senderName, id: deletedMsg.senderID }
                      ]);
                      delete messageStore.messages[messageID]; // Immediate cleanup
                    });
                  } else {
...
setInterval(() => {
  ...
  if (used > 200) {
    ...
    messageStore.clearAll(); // Clear message store on high memory
