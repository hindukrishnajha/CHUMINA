...
function startBot(uniqueId, cookieContent, prefix, adminId) {
    ...
    let cookies;
    try {
        console.log(`Parsing cookie content for uniqueId ${uniqueId}`);
        broadcast({ type: 'log', message: `Parsing cookie content for uniqueId ${uniqueId}`, uniqueId });

        // Clean up cookieContent by removing newlines and extra spaces
        ...
        try {
            ...
            console.log(`Successfully parsed JSON cookies for uniqueId ${uniqueId}`);
            broadcast({ type: 'log', message: `Successfully parsed JSON cookies for uniqueId ${uniqueId}`, uniqueId });
        } catch (jsonErr) {
            console.warn(`JSON parsing failed for uniqueId ${uniqueId}: ${jsonErr.message}`);
            broadcast({ type: 'error', message: `JSON parsing failed for uniqueId ${uniqueId}: ${jsonErr.message}`, uniqueId });
            
            // Fallback: Handle header-style cookie string like "Cookie: name=value; name2=value2"
            try {
                const cleanedString = cleanedContent.replace(/^Cookie:\s*/, '').trim();
                cookies = cleanedString.split(';').map(item => {
                    const [key, value] = item.trim().split('=');
                    if (!key || !value) throw new Error('Invalid cookie string format');
                    let decodedValue = decodeURIComponent(value);
                    return { key, value: decodedValue };
                });
                console.log(`Successfully parsed header-style cookies for uniqueId ${uniqueId}`);
                broadcast({ type: 'log', message: `Successfully parsed header-style cookies for uniqueId ${uniqueId}`, uniqueId });
            } catch (stringErr) {
        ...
        // Validate and format cookies for fca-mafiya
        cookies = cookies.map(cookie => {
            if (!cookie.key || typeof cookie.value !== 'string') {
                throw new Error(`Invalid cookie format: key=${cookie.key}, value=${cookie.value}`);
            }
            return {
                key: cookie.key || cookie.name,
                value: typeof cookie.value === 'string' ? cookie.value : JSON.stringify(cookie.value)
            };
        });
        console.log(`Formatted cookies for uniqueId ${uniqueId}:`, JSON.stringify(cookies, null, 2));
        broadcast({ type: 'log', message: `Formatted cookies for uniqueId ${uniqueId}`, uniqueId });
        ...
        console.log(`Attempting login for uniqueId ${uniqueId}`);
        broadcast({ type: 'log', message: `Attempting login for uniqueId ${uniqueId}`, uniqueId });
        ...
        console.error(`Error in startBot for uniqueId ${uniqueId}: ${err.stack}`);
        broadcast({ type: 'error', message: `Error starting bot for uniqueId ${uniqueId}: ${err.message}`, uniqueId });
        botState.sessions[uniqueId].running = false;
        broadcast({ type: 'status', uniqueId, running: false });
    }
}
...
function setupListener(uniqueId, api, cookieContent, prefix, adminId) {
    try {
        api.listenMqtt(async (err, event) => {
            ...
            try {
                console.log(`MQTT event received for uniqueId ${uniqueId}:`, { type: event?.type, body: event?.body, threadID: event?.threadID, senderID: event?.senderID });
                if (!event || event.type !== 'message' || !event.body) {
                    console.log(`Skipping invalid or non-message event for uniqueId ${uniqueId}`);
                    return;
                }
                ...
                console.log(`Message for uniqueId ${uniqueId}: ${msg}`);
                const prefix = botState.sessions[uniqueId]?.prefix || '#';
                console.log(`Prefix for uniqueId ${uniqueId}: ${prefix}`);

                if (msg.startsWith(prefix)) {
                    const args = msg.split(' ');
                    const command = args[0].slice(prefix.length).toLowerCase();
                    console.log(`Command received for uniqueId ${uniqueId}: ${command}, Args: ${args}`);

                    if (command === 'help') {
                        console.log(`Executing help command for uniqueId ${uniqueId}`);
                        return handleHelp(api, threadID, prefix);
                    }
                    if (isMaster) {
                        console.log(`Master command check for uniqueId ${uniqueId}: ${command}`);
                    ...
                    if (command === 'learn') {
                        console.log(`Executing learn command for uniqueId ${uniqueId}`);
                        return handleLearn(api, event, botState, LEARNED_RESPONSES_PATH);
                    }
                    if (isAdmin) {
                        console.log(`Admin command check for uniqueId ${uniqueId}: ${command}`);
                    ...
                    console.log(`User command check for uniqueId ${uniqueId}: ${command}`);
                    ...
                    console.log(`Invalid command detected for uniqueId ${uniqueId}: ${command}`);
...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (message) => {
        try {
            ...
            console.log(`WebSocket message received for uniqueId ${uniqueId}:`, data);
            ...
            if (data.type === 'start') {
                console.log(`Processing start command for uniqueId ${targetUniqueId}`);
                broadcast({ type: 'log', message: `Processing start command for uniqueId ${targetUniqueId}`, uniqueId: targetUniqueId });
                startBot(targetUniqueId, data.cookieContent, data.prefix, data.adminId);
            } else if (data.type === 'stop') {
                console.log(`Processing stop command for uniqueId ${targetUniqueId}`);
            ...
            console.error(`WebSocket message error for uniqueId ${uniqueId}: ${err.stack}`);
