...
// Load environment variables for abuse and welcome messages
if (process.env.ABUSE_BASE64) {
    try {
        const abuseContent = Buffer.from(process.env.ABUSE_BASE64, 'base64').toString('utf-8');
        // Only write default abuse.txt if it doesn't exist
        if (!fs.existsSync('abuse.txt')) {
            fs.writeFileSync('abuse.txt', abuseContent);
            console.log('Default abuse file created from environment variable');
        } else {
            console.log('Default abuse file already exists, skipping environment variable');
        }
    } catch (err) {
        console.error('Error creating default abuse file from environment variable:', err.message);
    }
}
...
// Cooldown function to prevent command spamming
...
function startBot(userId, cookieContent, prefix, adminID) {
    ...
    broadcast({ type: 'log', message: `Starting bot for user ${userId}...`, userId });
    if (botState.sessions[userId]) {
        stopBot(userId);
    }
            ...
            wiegine.login(cookieContent, {}, (err, api) => {
                if (err || !api) {
                    ...
                    if (attempt < maxAttempts) {
                        ...
                        broadcast({ 
                            type: 'error', 
                            message: `Max login attempts reached for user ${userId}. Please check cookies and try again.`, 
                            userId 
                        });
                        broadcast({ 
                            type: 'status', 
                            userId, 
                            running: false, 
                            message: `Max login attempts reached for user ${userId}. Bot failed to start.` 
                        });
                ...
                let abuseMessages = [];
                try {
                    // Load user-specific abuse file if exists, else fall back to default abuse.txt
                    const userAbuseFile = `abuse_${userId}.txt`;
                    abuseMessages = fs.existsSync(userAbuseFile) ? loadAbuseMessages(userAbuseFile) : loadAbuseMessages();
                    console.log(`Abuse messages loaded for user ${userId}: ${abuseMessages.length} messages`);
                ...
                const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 15) => {
                    ...
                    api.listenMqtt(async (err, event) => {
                        if (err) {
                            ...
                            broadcast({ type: 'error', message: `Listen error for user ${userId}: ${err?.message || err || 'Unknown error'}`, userId });
                            if (botState.sessions[userId]?.running && mqttAttempt < maxMqttAttempts) {
                                ...
                                broadcast({ type: 'error', message: `Max MQTT reconnection attempts reached for user ${userId}. Restarting bot.`, userId });
                        ...
                        if (['presence', 'read_receipt', 'message_reaction'].includes(event.type)) {
                            return; // Silently ignore non-essential events
                        }

                        // Handle undefined attachment types
                        if (event.attachments && event.attachments.some(att => att.type === undefined)) {
                            console.log(`[DEBUG] Ignored event with undefined attachment type: ${JSON.stringify(event, null, 2)}`);
                            return;
                        }
                        ...
                        try {
                            ...
                            if (event.type === 'message') {
                                ...
                                if (event.mentions && Object.keys(event.mentions).includes(botState.sessions[userId].adminID)) {
                                    ...
                                    api.sendMessage({ sticker: stickerID }, event.threadID, (err) => {
                                        if (err) {
                                            console.error(`Error sending sticker ${stickerID} to thread ${event.threadID}:`, err.message);
                                            broadcast({ type: 'error', message: `Failed to send sticker to thread ${event.threadID}: ${err.message}`, userId });
                                        } else {
                                            console.log(`[DEBUG] Sticker ${stickerID} sent to thread ${event.threadID}`);
                                        }
                                    });
                                }
...
👑 𝗖𝗿𝗲𝗮𝘁𝗲𝗱 𝗕𝗬: ✶♡⤾➝SHALENDER X..⤹✶➺🪿🫨🩷🪽`, threadID);
            ...
            broadcast({ type: 'error', message: `Error starting bot for user ${userId}: ${err.message}`, userId });
            broadcast({ type: 'status', userId, running: false, message: `Failed to start bot for user ${userId}: ${err.message}` });
            botState.sessions[userId].running = false;
        }
    };
...
process.on('uncaughtException', (err) => {
    ...
    broadcast({ type: 'error', message: `Uncaught Exception: ${err.message}`, userId: 'system' });
});

process.on('unhandledRejection', (reason, promise) => {
    console.error('Unhandled Rejection at:', promise, 'reason:', reason);
    broadcast({ type: 'error', message: `Unhandled Rejection: ${reason}`, userId: 'system' });
});
    ...
    ws.on('message', (message) => {
        try {
            ...
            if (!messageString) {
                ...
                ws.send(JSON.stringify({ type: 'error', message: 'Error: Empty message received' }));
                return;
            }
            ...
            if (data.type === 'start') {
                ...
                if (data.userId) {
                    if (botState.sessions[data.userId]) {
                        ...
                        ws.send(JSON.stringify({ type: 'status', userId: data.userId, running: false, message: `Bot stopped for user ${data.userId}` }));
                    } else {
                        ws.send(JSON.stringify({ type: 'error', message: `No active session for user ${data.userId}`, userId: data.userId }));
                        ws.send(JSON.stringify({ type: 'status', userId: data.userId, running: false, message: `No active session for user ${data.userId}` }));
                    }
                } else {
                    ws.send(JSON.stringify({ type: 'error', message: 'Invalid userId provided' }));
                }
                ...
                ws.send(JSON.stringify({ type: 'status', userId, running, message: running ? `Bot is running for user ${userId}` : `Bot is not running for user ${userId}` }));
            } else if (data.type === 'uploadAbuse') {
                console.log(`Received uploadAbuse request for user ${data.userId}`);
                try {
                    const userAbuseFile = `abuse_${data.userId}.txt`;
                    saveFile(userAbuseFile, data.content);
                    console.log(`User-specific abuse file ${userAbuseFile} saved successfully`);
                    ws.send(JSON.stringify({ type: 'log', message: `Abuse messages updated successfully for user ${data.userId}`, userId: data.userId }));
                } catch (err) {
                    console.error(`Error saving user-specific abuse file for user ${data.userId}:`, err.message);
                    ws.send(JSON.stringify({ type: 'error', message: `Failed to update abuse messages for user ${data.userId}: ${err.message}`, userId: data.userId }));
                }
            ...
            ws.send(JSON.stringify({ type: 'error', message: `Error processing WebSocket message: ${err.message}` }));
        }
    });
