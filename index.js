...
function startBot(userId, cookieContent, prefix, adminID) {
    ...
    botState.sessions[userId] = {
        ...
        adminID: adminID || '',
        api: null,
        cookieContent // Store cookie for reconnection
    };
            ...
            wiegine.login(cookieContent, {}, (err, api) => {
                if (err || !api) {
                    ...
                    if (attempt < maxAttempts) {
                        console.log(`Retrying login for user ${userId} in 10 seconds (attempt ${attempt + 1})`);
                        setTimeout(() => tryLogin(attempt + 1, maxAttempts), 10000);
                    } else {
                    ...
                    api.listenMqtt(async (err, event) => {
                        if (err) {
                            ...
                            if (botState.sessions[userId].running) {
                                console.log(`Reconnecting MQTT for user ${userId} in 10 seconds`);
                                ...
                                if (msg.startsWith(botState.sessions[userId].prefix)) {
                                    ...
                                    if (isAdmin) {
                                        if (command === 'groupnamelock') {
                                            ...
                                            handleStickerSpam(api, event, botState, favoriteStickers);
                                            return;
                                    ...
                                    // Handle mixed commands like "#unsend sticker start 800"
                                    if (command === 'unsend' && args[1] && args[1].toLowerCase() === 'sticker') {
                                        api.sendMessage('❌ Did you mean "#send sticker start <count>"? Please use the correct command.', threadID, messageID);
                                        console.log(`[DEBUG] Mixed command detected: ${event.body} in thread ${threadID}`);
                                        return;
                                    }
