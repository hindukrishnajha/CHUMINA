...
function stopBot(userId) {
  ...
  // मैन्युअली स्टॉप फ्लैग सेट करें
...
function startBot(userId, cookieContent, prefix, adminID) {
  ...
  botState.sessions[userId] = {
    ...
    manualStop: false // स्टार्ट पर मैन्युअली स्टॉप फ्लैग रीसेट
  };

  const tryLogin = (attempt = 1, maxAttempts = 2) => {
    try {
      const cookieFile = `cookies_${userId}.txt`;
      // कुकीज फाइल चेक करें, अगर पहले से मौजूद है तो उसे यूज करें
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 3) => {
          ...
          api.listenMqtt(async (err, event) => {
            if (err) {
              ...
              if (botState.sessions[userId]?.running && !botState.sessions[userId]?.manualStop && mqttAttempt < maxMqttAttempts) {
                setTimeout(() => listenMqtt(mqttAttempt + 1, maxMqttAttempts), 10000 * mqttAttempt);
              } else {
                console.error(`Max MQTT reconnection attempts reached or manually stopped for user ${userId}`);
                botState.sessions[userId].running = false;
                delete botState.sessions[userId];
                broadcast({ type: 'log', message: `MQTT reconnection failed or bot manually stopped for user ${userId}`, userId });
              }
              return;
            }
