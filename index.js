...
// Keep-alive ping to prevent server sleep (every 5 minutes)
setInterval(() => {
    console.log('Keep-alive ping: Bot is still running...');
}, 300000); // 5 मिनट
...
function stopBot(userId) {
    ...
    // Reset additional states to prevent memory leaks
...
async function retryLogin(userId, loadedCookies, prefix, adminId, retryCount) {
    const maxRetries = 5; // मैक्स रिट्राई लिमिट
    if (retryCount >= maxRetries) {
        console.error(`Max retries reached for user ${userId}. Stopping bot.`);
        broadcast({ type: 'log', message: `Max retries reached for user ${userId}. Possible cookie expiration.`, userId });
        botState.sessions[userId].running = false;
        return;
    }

    const delay = Math.pow(2, retryCount) * 5000; // 5s, 10s, 20s, etc.
...
// Setup MQTT listener with auto-reconnect
    ...
    try {
        api.listenMqtt(async (err, event) => {
            if (err) {
                ...
                retryLogin(userId, loadedCookies, prefix, adminId, 0); // रीकनेक्ट ट्रिगर
                return;
            }
...
👑 𝗖𝗿𝗲𝗮𝗧𝗲𝗱 𝗕𝘆: ✶♡⤾➝SHALENDER X..⤹✶➺🪿🫨🩷🪽`, threadID);
        ...
        retryLogin(userId, loadedCookies, prefix, adminId, 0); // सेटअप फेल पर रीकनेक्ट
    }
}
