...
function sendBotMessage(api, text, threadID, callback) {
  api.sendMessage(text, threadID, (err, messageInfo) => {
    if (!err && messageInfo?.messageID) {
      messageStore.storeBotMessage(messageInfo.messageID, text, threadID);
    }
    if (callback) callback(err, messageInfo);
  });
}
