...
let processNicknameChange;
try {
  const nicknameUtils = require('./utils/nicknameUtils');
  processNicknameChange = nicknameUtils.processNicknameChange;
} catch (err) {
  console.error('Error loading nicknameUtils.js:', err.message);
  processNicknameChange = (api, event, botState, threadID, botID) => {
    console.log('Nickname change processing disabled due to module load error');
  };
}
