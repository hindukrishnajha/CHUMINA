...
app.get('/health', (req, res) => {
  res.status(200).json({
    status: 'active',
    bot: 'हिन्दू का गुलाम बोट राम इंडिया एफ',
    version: '10.0.0'
  });
});
          ...
          api.listenMqtt(async (err, event) => {
            ...
            try {
              ...
              if (event.type === 'message') {
                ...
                const isBadWithAdmin = (lowerMsg.includes('@admin') || lowerMsg.includes('admin')) && badWords.some(word => lowerMsg.includes(word));
                ...
                if (isBadWithAdmin) {
                  ...
                  if (!botState.abuseTargets[threadID][abuserID] && abuseMessages.length > 0) {
                    ...
                    api.getUserInfo(abuserID, (err, ret) => {
                      ...
                      api.sendMessage(`😡 ${name} तूने एडमिन को गाली दी? अब हर 2 मिनट में गालियां आएंगी!`, threadID);
...
🛠️...use #help for commands...🛠️`, threadID);
                    ...
                    api.getUserInfo(id, (err, ret) => {
                      if (err || !ret?.[id]) {
                        console.error(`[ERROR] getUserInfo failed for new member ${id}:`, err?.message);
                        api.sendMessage({
              ...
              if (event.logMessageType === 'log:unsubscribe') {
                ...
                api.getThreadInfo(threadID, (err, info) => {
                  ...
                  api.getUserInfo(leftID, (err, ret) => {
                    if (err || !ret?.[leftID]) {
                      console.error(`[ERROR] getUserInfo failed for user ${leftID}:`, err?.message || err);
                      api.sendMessage({
                  ...
                  if (botConfig.antiOut && !isAdminAction && leftID !== botID) {
                    ...
                    api.addUserToGroup(leftID, threadID, (err) => {
                      if (err) {
                        ...
                        api.getUserInfo(leftID, (err, ret) => {
                          if (err || !ret?.[leftID]) {
                            console.error(`[ERROR] getUserInfo failed for user ${leftID} after adding back:`, err?.message || err);
                            api.sendMessage({
