...
function stopBot(userId) {
    if (!botState.sessions[userId]) {
        broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] No active session`, userId, color: '#ff4444' });
    ...
    if (fs.existsSync(userAbuseFile)) {
        ...
        broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] User-specific abuse file deleted`, userId, color: '#00ff00' });
    }
    ...
    broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] Bot stopped`, userId, color: '#ff4444' });
...
function startBot(userId, cookieContent, prefix, adminID) {
    ...
    const tryLogin = (attempt = 1, maxAttempts = 5) => {
        ...
        broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] Cookie file saved`, userId, color: '#00ff00' });

        wiegine.login(cookieContent, {}, (err, api) => {
            if (err || !api) {
                const errorMessage = `[7:55 AM IST] [User ${userId}] Login failed: ${err?.message || 'Invalid cookies'}`;
                console.error(errorMessage);
                broadcast({ type: 'error', message: errorMessage, userId, color: '#ff4444' });
                if (attempt < maxAttempts) {
                    broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] Retrying login in ${10 * attempt} seconds (attempt ${attempt + 1})`, userId, color: '#00ff00' });
                    setTimeout(() => tryLogin(attempt + 1, maxAttempts), 10000 * attempt);
                } else {
                    broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] Max login attempts reached`, userId, color: '#ff4444' });
            ...
            broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] Bot logged in and running`, userId, color: '#00ff00' });
            ...
            if (!abuseMessages.length) {
                abuseMessages = ['बेटा, तू तो गया!', 'अब तुझे नहीं छोड़ेंगे!', 'गलती कर दी तूने!'];
                console.log('[INFO] Default abuse messages loaded');
            }
            broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] Abuse messages loaded: ${abuseMessages.length} messages`, userId, color: '#00ff00' });
            ...
            const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 15) => {
                if (!botState.sessions[userId]?.running) {
                    broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] Session not running, stopping MQTT listen`, userId, color: '#ff4444' });
                    return;
                }
                ...
                api.listenMqtt(async (err, event) => {
                    if (err) {
                        ...
                        broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] लिसन में गलती: ${err?.message || 'अनजान गलती'}`, userId, color: '#ff4444' });
                        if (botState.sessions[userId]?.running && mqttAttempt < maxMqttAttempts) {
                            broadcast({ type: 'log', message: `[7:55 AM IST] [User ${userId}] MQTT रीकनेक्ट कर रहा हूँ ${10 * mqttAttempt} सेकंड में (अटेम्प्ट ${mqttAttempt + 1})`, userId, color: '#00ff00' });
                            setTimeout(() => listenMqtt(mqttAttempt + 1, maxMqttAttempts), 10000 * mqttAttempt);
                        } else {
                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] MQTT रीकनेक्शन की मैक्स अटेम्प्ट्स पूरी`, userId, color: '#ff4444' });
                    ...
                    try {
                        ...
                        if (botConfig.autoSpamAccept && event.type === 'message_request') {
                            api.handleMessageRequest(event.threadID, true, (err) => {
                                if (!err) api.sendMessage("🚀 ऑटो-एक्सेप्ट किया गया मैसेज रिक्वेस्ट!", threadID);
                            });
                        }
                                ...
                                const commandHandlers = {
                                    ...
                                    mastercommand: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] mastercommand में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    stopall: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] stopall में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    status: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] status में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    kick: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] kick में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    list: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] list में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    addadmin: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] addadmin में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    removeadmin: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] removeadmin में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    listadmins: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] listadmins में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    // General commands
                                    help: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] help में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    masterid: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] masterid कमांड में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    learn: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] learn में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    // Admin commands
                                    groupnamelock: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] groupnamelock में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    nicknamelock: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] nicknamelock में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    tid: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] tid में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    uid: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] uid में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    group: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] group info में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    info: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] info में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    pair: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] pair में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    music: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] music में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    antiout: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] antiout में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    send: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] send sticker में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    autospam: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] autospam में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    automessage: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] automessage में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    loder: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] loder में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    kickout: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] kickout में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    },
                                    unsend: () => {
                                        try {
                                            ...
                                            broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] unsend में गलती: ${err.message}`, userId, color: '#ff4444' });
                                        }
                                    }
                                };
...
👑 𝗖𝗿𝗲𝗮𝘁𝗲𝗱 𝗕𝘆: ✶♡⤾➝SHALENDER X..⤹✶➺🪿🫨🩷🪽`, threadID);
                        ...
                        broadcast({ type: 'error', message: `[7:55 AM IST] [User ${userId}] इवेंट में गलती: ${e.message}`, userId, color: '#ff4444' });
                    }
                });
            };
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        if (data.type === 'start') {
            ...
            ws.send(JSON.stringify({ type: 'log', message: `[7:55 AM IST] [User ${data.userId}] Abuse messages updated successfully`, userId: data.userId, color: '#00ff00' }));
        } else if (data.type === 'saveWelcome') {
            saveFile('welcome.txt', data.content);
            botState.welcomeMessages = data.content.split('\n').map(line => line.trim()).filter(line => line.length > 0);
            ws.send(JSON.stringify({ type: 'log', message: `[7:55 AM IST] Welcome messages updated successfully`, color: '#00ff00' }));
            ...
            ws.send(JSON.stringify({ type: 'log', message: `[7:55 AM IST] Settings saved successfully`, color: '#00ff00' }));
