...
const { getAIResponse } = require('./utils/aichat'); // Groq AI इम्पोर्ट

// Ensure UTF-8 encoding globally for Hindi text handling
...
if (!botState.eventProcessed) botState.eventProcessed = {}; // Prevent duplicate event processing
if (!botState.chatEnabled) botState.chatEnabled = {}; // नया: AI चैट स्टेट
...
// Environment variables load - Explicit UTF-8 for Hindi
...
// Stop bot function - Enhanced user data deletion
function stopBot(userId) {
  ...
  // Delete user-specific data (sessions, etc.)
  ...
  // Clear any user-related caches if tied to threads
      ...
      wiegine.login(cookieContent, {}, (err, api) => {
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 3) => {
          ...
          api.listenMqtt(async (err, event) => {
            ...
            // Prevent duplicate event processing with size limit
            ...
            if (event.messageID) {
              if (Object.keys(botState.eventProcessed).length > 1000) {
                ...
                console.log('[DEBUG] Cleared eventProcessed cache due to size limit');
              }
              botState.eventProcessed[event.messageID] = true;
              setTimeout(() => delete botState.eventProcessed[event.messageID], 60000);
            }
            ...
            try {
              ...
              if (!senderID) {
                ...
                console.log('Event data:', JSON.stringify(event)); // डिबगिंग के लिए
                return;
              }
...
🛠️...use #help for commands...🛠️`, threadID);
              ...
              if (event.type === 'message') {
                ...
                if (msg.startsWith(botState.sessions[userId].prefix)) {
                  ...
                  if (cmd) {
                    // डबल प्रोसेसिंग रोकने के लिए चेक
                ...
                const replyList = autoreplies.autoreplies; // autoreplies.js से सही रेफरेंस
                ...
                // चैट स्टेट चेक करने के लिए डिफॉल्ट वैल्यू
                if (!botState.chatEnabled) {
                  botState.chatEnabled = {};
                }

                const specialTags = ['#ai', '@bot'];
                if (!responseSent && !msg.startsWith(botState.sessions[userId].prefix) && specialTags.some(tag => msg.toLowerCase().includes(tag))) {
                  if (botState.chatEnabled[threadID] === true) { // सिर्फ ऑन होने पर AI चले
                    try {
                      const userMessage = event.body.replace(/#ai|@bot/gi, '').trim();
                      const groqResponse = await getAIResponse(userMessage || 'हाय भाई, कुछ बोलो!');
                    ...
                    api.sendMessage('❌ मालिक, चैट ऑफ है! केवल कमांड्स वर्क करेंगी।', threadID);
                    responseSent = true;
                  }
                }
              }
...
// Keep-alive - More aggressive for Render free tier (15 sec)
const keepAlive = setInterval(() => {
  axios.get(`https://${process.env.RENDER_SERVICE_NAME}.onrender.com/health`).catch(err => {});
}, 15000);

// Memory monitoring - More aggressive (1 min interval, 200MB threshold)
setInterval(() => {
