...
if (!botState.learnedResponses) botState.learnedResponses = {}; // User-specific learned responses
...
try {
  if (fs.existsSync(LEARNED_RESPONSES_PATH)) {
    ...
    // Ensure each user has a triggers array
...
// Stop bot function (Updated: Clear user-specific data including learned responses)
function stopBot(userId) {
  ...
  botState.sessions[userId].manualStop = true; // Prevent retry

  // Clear user-specific data
  if (botState.learnedResponses[userId]) {
    botState.learnedResponses[userId].triggers = []; // Clear user-specific learned responses
    fs.writeFileSync(LEARNED_RESPONSES_PATH, JSON.stringify(botState.learnedResponses, null, 2));
  }
...
// Start bot function (Updated: Initialize user-specific learned responses)
function startBot(userId, cookieContent, prefix, adminID) {
  ...
  // Initialize user-specific learned responses
  if (!botState.learnedResponses[userId]) {
    botState.learnedResponses[userId] = { triggers: [] };
    fs.writeFileSync(LEARNED_RESPONSES_PATH, JSON.stringify(botState.learnedResponses, null, 2));
  }
          ...
          api.listenMqtt(async (err, event) => {
            if (err) {
              ...
              if (mqttAttempt < maxMqttAttempts && !botState.sessions[userId]?.manualStop) {
                ...
                return; // Keep bot running without retry
              }
              return;
            }
            ...
            try {
              ...
              // Member caching (limited to save memory)
...
🛠️...use #help for commands...🛠️`, threadID);
              ...
              if (event.type === 'message') {
                ...
                if (botState.learnedResponses[userId]?.triggers) {
                  for (const { trigger, response } of botState.learnedResponses[userId].triggers) {
                      ...
                      try {
                        api.getUserInfo(targetID, (err, ret) => {
                          ...
                          const spamLoop = async () => {
                            let attempts = 0;
                            const maxAttempts = 10; // 20 minutes
                            while (botState.abuseTargets[threadID]?.[targetID] && attempts < maxAttempts && abuseMessages.length > 0) {
                              try {
                                ...
                                break; // Stop loop, keep bot running
                              }
                            }
