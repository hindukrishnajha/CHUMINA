...
if (!botState.removeNicknameActive) botState.removeNicknameActive = {}; // New for remove nickname mode
if (!botState.removeNicknameTargets) botState.removeNicknameTargets = {}; // New for specific user remove mode
...
function stopBot(userId) {
  ...
  Object.keys(botState.lockedNicknames).forEach(threadID => {
    // Assuming threads are per user bot, delete entire thread data
    delete botState.lockedNicknames[threadID];
  });

  Object.keys(botState.removeNicknameActive).forEach(threadID => {
    delete botState.removeNicknameActive[threadID];
  });

  Object.keys(botState.removeNicknameTargets).forEach(threadID => {
