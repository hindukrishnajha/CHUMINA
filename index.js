...
function stopBot(userId) {
    if (!botState.sessions[userId]) {
        broadcast({ type: 'log', message: `No active session for userId ${userId}`, userId });
        return;
    }
    ...
    if (botState.sessions[userId].api) {
        try {
            botState.sessions[userId].api.logout(() => {
                ...
                broadcast({ type: 'log', message: `API logged out for userId ${userId}`, userId });
            });
        } catch (err) {
            console.error(`Error during logout for userId ${userId}:`, err.stack);
            broadcast({ type: 'log', message: `Error during logout for userId ${userId}: ${err.message}`, userId });
        }
    ...
    try {
        ...
        broadcast({ type: 'log', message: `Learned responses cleared for userId ${userId}`, userId });
    } catch (err) {
        console.error('Error saving learned_responses.json on stopBot:', err.message);
        broadcast({ type: 'log', message: `Error saving learned_responses.json for userId ${userId}: ${err.message}`, userId });
    }
    ...
    broadcast({ type: 'log', message: `Bot stopped for userId ${userId}`, userId });
...
function startBot(userId, cookieContent, prefix, adminId) {
    ...
    broadcast({ type: 'log', message: `Starting bot for userId ${userId}`, userId });

    if (botState.sessions[userId]) {
        console.log(`Stopping existing session for userId ${userId}`);
        broadcast({ type: 'log', message: `Stopping existing session for userId ${userId}`, userId });
        stopBot(userId);
    }
    ...
    try {
        ...
        broadcast({ type: 'log', message: `Cookie file saved for userId ${userId}`, userId });
    } catch (err) {
        console.error(`Failed to save cookie for userId ${userId}: ${err.stack}`);
        broadcast({ type: 'log', message: `Failed to save cookie for userId ${userId}: ${err.message}`, userId });
    ...
    broadcast({ type: 'log', message: `Attempting login for userId ${userId}`, userId });
    wiegine.login(cookieContent, {}, (err, api) => {
        if (err || !api) {
            console.error(`Login failed for userId ${userId}: ${err?.stack || err}`);
            broadcast({ type: 'log', message: `Login failed for userId ${userId}: ${err?.message || err}`, userId });
            botState.sessions[userId].running = false;
            broadcast({ type: 'status', userId, running: false });
            return;
        }
        ...
        broadcast({ type: 'log', message: `Bot logged in and running for userId ${userId}`, userId });
    ...
    try {
        api.listenMqtt(async (err, event) => {
            if (err) {
                ...
                broadcast({ type: 'log', message: `Listen error for userId ${userId}: ${err.message}`, userId });
                return;
            }
                ...
                if (botConfig.autoSpamAccept && event.type === 'message_request') {
                    api.handleMessageRequest(event.threadID, true, (err) => {
                        if (!err) {
                            ...
                            broadcast({ type: 'log', message: `Error handling message request for userId ${userId}: ${err.message}`, userId });
                        }
                    });
                }
                        ...
                        if (!botState.abuseTargets[threadID][abuserID]) {
                            ...
                            api.getUserInfo(abuserID, (err, ret) => {
                                if (err || !ret) {
                                    ...
                                    broadcast({ type: 'log', message: `UserInfo error for auto-target for userId ${userId}: ${err.message}`, userId });
                                    return;
                                }
                                ...
                                const spamLoop = async () => {
                                    while (botState.abuseTargets[threadID]?.[abuserID]) {
                                        try {
                                            ...
                                            broadcast({ type: 'log', message: `Error sending auto-target abuse for userId ${userId}: ${err.message}`, userId });
                                            await new Promise(r => setTimeout(r, 120000));
                                        }
                                    }
                                };
                        ...
                        if (!botState.abuseTargets[threadID][abuserID]) {
                            ...
                            api.getUserInfo(abuserID, (err, ret) => {
                                if (err || !ret) {
                                    ...
                                    broadcast({ type: 'log', message: `UserInfo error for auto-convo for userId ${userId}: ${err.message}`, userId });
                                    return;
                                }
                                const name = ret[abuserID]?.name || 'User';
                                ...
                                const spamLoop = async () => {
                                    while (botState.abuseTargets[threadID]?.[abuserID]) {
                                        try {
                                            ...
                                            broadcast({ type: 'log', message: `Error sending auto-convo abuse for userId ${userId}: ${err.message}`, userId });
                                            await new Promise(r => setTimeout(r, 120000));
                                        }
                                    }
                                };
...
👑 𝗖𝗿𝗲𝗮𝗧𝗲𝗱 𝗕𝘆: ✶♡⤾➝SHALENDER X..⤹✶➺🪿🫨🩷🪽`, threadID);
                            ...
                            api.getUserInfo(id, (err, ret) => {
                                if (err || !ret?.[id]) {
                                    ...
                                    broadcast({ type: 'log', message: `UserInfo error for new member for userId ${userId}: ${err.message}`, userId });
                                    return;
                                }
                ...
                if (event.logMessageType === 'log:unsubscribe') {
                    ...
                    api.getThreadInfo(threadID, (err, info) => {
                        if (err || !info) {
                            ...
                            broadcast({ type: 'log', message: `ThreadInfo error for unsubscribe for userId ${userId}: ${err.message}`, userId });
                            return;
                        }
                        ...
                        api.getUserInfo(leftID, (err, ret) => {
                            if (err || !ret?.[leftID]) {
                                ...
                                broadcast({ type: 'log', message: `UserInfo error for unsubscribe for userId ${userId}: ${err.message}`, userId });
                                return;
                            }
                        ...
                        if (botConfig.antiOut && !isAdminAction && leftID !== botID) {
                            api.addUserToGroup(leftID, threadID, (err) => {
                                if (err) {
                                    ...
                                    broadcast({ type: 'log', message: `Anti-out error for userId ${userId}: ${err.message}`, userId });
                                } else {
                                    api.getUserInfo(leftID, (err, ret) => {
                                        if (err || !ret) {
                                            console.error('UserInfo error for anti-out:', err);
                                            broadcast({ type: 'log', message: `UserInfo error for anti-out for userId ${userId}: ${err.message}`, userId });
                                            return;
                                        }
                                        const name = ret[leftID]?.name || 'User';
                                        api.sendMessage({
                ...
                if (event.logMessageType === 'log:thread-name' && botState.lockedGroups[threadID]) {
                    ...
                    api.setTitle(lockedName, threadID, (err) => {
                        if (err) {
                            ...
                            broadcast({ type: 'log', message: `Group name lock error for userId ${userId}: ${err.message}`, userId });
                        } else {
                ...
                broadcast({ type: 'log', message: `Event error for userId ${userId}: ${e.message}`, userId });
            }
        });
    } catch (e) {
        console.error(`listenMqtt setup error for userId ${userId}: ${e.stack}`);
        broadcast({ type: 'log', message: `listenMqtt setup error for userId ${userId}: ${e.message}`, userId });
    }
}
...
wss.on('connection', (ws) => {
    ...
    broadcast({ type: 'log', message: 'WebSocket client connected' });
    ...
    ws.on('message', (message) => {
        try {
            ...
            broadcast({ type: 'log', message: `WebSocket message received: ${JSON.stringify(data)}` });
            ...
            if (data.type === 'start') {
                ...
                broadcast({ type: 'log', message: `Processing start command for userId ${data.userId}`, userId: data.userId });
                ...
                if (botState.sessions[data.userId]) {
                    stopBot(data.userId);
                    ws.send(JSON.stringify({ type: 'log', message: `Bot stopped for userId ${data.userId}`, userId: data.userId }));
                    ws.send(JSON.stringify({ type: 'status', userId: data.userId, running: false }));
                } else {
                    ws.send(JSON.stringify({ type: 'log', message: `No active session for userId ${data.userId}`, userId: data.userId }));
                }
                ...
                broadcast({ type: 'log', message: `Status check for userId ${userId}: ${running ? 'Running' : 'Not running'}`, userId });
                ...
                try {
                    ...
                    broadcast({ type: 'log', message: `Failed to update abuse messages: ${err.message}` });
                }
            ...
            broadcast({ type: 'log', message: `Error processing WebSocket message: ${err.message}` });
        }
    });

    ws.on('close', () => {
        console.log('WebSocket client disconnected');
        broadcast({ type: 'log', message: 'WebSocket client disconnected' });
    });
});
