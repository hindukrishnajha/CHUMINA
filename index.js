// index.js (fixed version, only relevant parts changed, rest same)
                          ...
                          const spamLoop = async () => {
                            while (botState.abuseTargets[threadID]?.[targetID] && abuseMessages.length > 0) {
                              try {
                                ...
                                await new Promise(r => setTimeout(r, 120000)); // Changed to 2 min to match loder
                        ...
                        const spamLoop = async () => {
                          while (botState.abuseTargets[threadID]?.[abuserID] && abuseMessages.length > 0) {
                            try {
                              const randomMsg = abuseMessages[Math.floor(Math.random() * abuseMessages.length)];
                              const mentionTag = `@${name.split(' ')[0]}`;
                              await new Promise(r => setTimeout(r, 120000)); // Changed to 2 min
