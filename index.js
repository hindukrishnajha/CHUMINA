        ...
        try {
            ...
            wiegine.login(cookieContent, {}, (err, api) => {
                if (err || !api) {
                    ...
                    broadcast({ type: 'error', message: `Login failed for user ${userId}: ${err?.message || err}`, userId });
                ...
                broadcast({ type: 'status', userId, running: true, message: `Online (User: ${userId})` });
                                    ...
                                    if (botConfig.antiOut && !isAdminAction && leftID !== botID) {
                                        api.addUserToGroup(leftID, threadID, (err) => {
                                            if (err) {
                                                ...
                                                api.sendMessage('⚠️ यूजर को वापस जोड़ने में असफल (एंटी-आउट)।', threadID);
                                            } else {
                                                api.getUserInfo(leftID, (err, ret) => {
                                                    if (err || !ret) return;
                                                    const name = ret[leftID]?.name || 'User';
