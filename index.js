...
const { getAIResponse } = require('./utils/aichat'); // नया: Groq AI इम्पोर्ट
...
function stopBot(userId) {
  ...
  // Clear any user-related caches if tied to threads (but keep global for other users)
  // Note: Thread-based caches like abuseTargets, memberCache are shared but will be cleared on memory interval anyway
    ...
    try {
      ...
      wiegine.login(cookieContent, {}, (err, api) => {
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 3) => {
          ...
          api.listenMqtt(async (err, event) => {  // नया: async बनाया ताकि await यूज हो सके
            if (err) {
            ...
            if (event.messageID) {
              // Size limit for eventProcessed to avoid memory leak
...
🛠️...use #help for commands...🛠️`, threadID);
              ...
              if (event.type === 'message') {
                ...
                // नया: अगर कोई रिस्पॉन्स न मिला (कमांड न हो, कोई मैच न हो), तो Groq AI से नॉर्मल जवाब लें
                if (!responseSent && !msg.startsWith(botState.sessions[userId].prefix)) {
                  try {
                    const groqResponse = await getAIResponse(event.body); // मूल मैसेज भेजें (lowercase नहीं)
                    api.sendMessage(groqResponse, threadID, messageID);
                    responseSent = true;
                  } catch (err) {
                    console.error('Error calling Groq AI:', err.message);
                    // कोई डिफॉल्ट मैसेज नहीं भेजें, चुप रहें
                  }
                }
              }
