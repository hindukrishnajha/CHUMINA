...
const crypto = require('crypto'); // For generating random userId if needed
...
wss.on('connection', (ws, req) => {
    ...
    let userId = query.userId || crypto.randomBytes(8).toString('hex'); // Use query.userId or generate random if not provided
    ...
    ws.on('error', (err) => {
        console.error(`WebSocket connection error for user ${userId}:`, err.stack);
        broadcast({ type: 'error', message: `WebSocket connection error for user ${userId}: ${err.message}`, userId });
    });
    ...
    ws.on('message', (message) => {
        try {
            ...
            // Use the ws.userId directly, no matching with data.userId
            const targetUserId = userId; // Always use the connected userId, ignore data.userId if mismatched

            if (data.type === 'start') {
                broadcast({ type: 'log', message: `Starting bot for user ${targetUserId}` });
                startBot(targetUserId, data.cookieContent, data.prefix, data.adminId);
                ...
                try {
                    ...
                    broadcast({ type: 'log', message: 'Abuse messages updated successfully', userId: targetUserId });
                } catch (err) {
                    ws.send(JSON.stringify({ type: 'error', message: `Failed to update abuse messages: ${err.message}`, userId: targetUserId }));
                }
            } else if (data.type === 'saveWelcome') {
                try {
                    fs.writeFileSync('welcome.txt', data.content);
                    botState.welcomeMessages = loadWelcomeMessages();
                    broadcast({ type: 'log', message: 'Welcome messages updated successfully', userId: targetUserId });
                } catch (err) {
                    ws.send(JSON.stringify({ type: 'error', message: `Failed to update welcome messages: ${err.message}`, userId: targetUserId }));
                }
            }
        } catch (err) {
