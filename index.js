...
try {
  ...
  botState.learnedResponses = {Exit adminList: [MASTER_ID], chatEnabled: {}, deleteNotifyEnabled: {} };
  botState.adminList = [MASTER_ID];
  botState.chatEnabled = {};
  botState.deleteNotifyEnabled = {};
  fs.writeFileSync(LEARNED_RESPONSES_PATH, JSON.stringify(botState.learnedResponses, null, 2), 'utf8');
...
function sendBotMessage(api, message, threadID, callback, mentions = []) {
  ...
  if (botState.sessions[userId]?.safeMode) {
    console.log(`[SAFE] Skipping message to ${threadID} due to SAFE MODE`);
...
function startBot(userId, cookieContent, prefix, adminID) {
  ...
  const tryLogin = (attempt = 1, maxAttempts = 1) => {
    ...
    if (attempt > maxAttempts) {
      console.error(`[SAFE] Login failed for ${userId} after ${maxAttempts} attempts`);
      ...
      wiegine.login(cookieContent, {}, (err, api) => {
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 1) => {
          ...
          api.listenMqtt(async (err, event) => {
            if (err) {
              console.error(`[SAFE] Listen error for ${userId} (attempt ${mqttAttempt}):`, err?.message || err);
              botState.sessions[userId].safeMode = true;
            ...
            if (event.type === 'read_receipt' || event.type === 'presence') {
              console.log(`[DEBUG] Skipping event type: ${event.type} for threadID=${event.threadID}`);
              return;
            }

            if (event.messageID && botState.eventProcessed[event.messageID] && event.type !== 'message_unsend') {
              console.log(`[DEBUG] Skipping duplicate event: ${event.messageID}`);
              return;
            }
            if (event.messageID) {
              if (Object.keys(botState.eventProcessed).length > 200) { // 100 से 200
                botState.eventProcessed = {};
            ...
            try {
              ...
              if (event.type === 'message_unsend' && botState.deleteNotifyEnabled[threadID]) {
                ...
                const tryThreadInfo = (attempt = 1, maxAttempts = 2) => {
                  api.getThreadInfo(threadID, (err, info) => {
                    if (err && attempt < maxAttempts) {
                      console.error(`[ERROR] Thread info attempt ${attempt} failed for ${threadID}: ${err.message}`);
                      setTimeout(() => tryThreadInfo(attempt + 1, maxAttempts), 2000);
                      return;
                    }
                    if (err || !info) {
                      console.error('[ERROR] Failed to fetch thread info for unsend:', err?.message || 'Unknown error');
                    ...
                    if (deletedMsg) {
                      ...
                      console.log(`[ERROR] No message found for unsend event: messageID=${messageID}, threadID=${threadID}`);
              ...
              if (event.type === 'message') {
                ...
                await messageStore.storeMessage(messageID, content, senderID, threadID, attachment); // Async स्टोरेज
                          ...
                          const spamLoop = async () => {
                            while (botState.abuseTargets[threadID]?.[targetID] && abuseMessages.length > 0) {
                              try {
                                ...
                                await new Promise(r => setTimeout(r, 180000)); // 3 मिनट फिक्स्ड
