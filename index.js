                  ...
                  if (cmd) {
                    try {
                      if (['stickerspam', 'antiout', 'groupnamelock', 'nicknamelock'].includes(cmd.name) && !isAdmin) {
                        ...
                        cmd.execute(api, threadID, args.slice(1), event, botState, isMaster); // Changed to args.slice(1)
                      }
                    } catch (err) {
                ...
                if (botState.lockedNicknames?.[threadID]?.[changedUserID]) {
                  ...
                  if (event.logMessageData.nickname !== lockedNickname) {
                    api.changeNickname(lockedNickname, threadID, changedUserID, (err) => {
                      if (err) {
                        console.error(`[ERROR] Failed to restore nickname for ${changedUserID}: ${err.message}`);
                        api.sendMessage('⚠️ निकनेम रीस्टोर करने में गलती।', threadID);
                        return;
                      }
                      api.getUserInfo(changedUserID, (err, ret) => {
                        const name = ret?.[changedUserID]?.name || 'User';
                        api.sendMessage(`🔒 ${name} का निकनेम "${lockedNickname}" पर वापस सेट किया गया!`, threadID);
