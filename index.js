...
async function startBot(userId, cookieContent, prefix, adminId, retryCount = 0) {
    if (botState.sessions[userId]) {
        stopBot(userId);
    }
    ...
    try {
        ...
        let parsedCookies;
        try {
            parsedCookies = JSON.parse(loadedCookies);
            console.log(`Cookies loaded for user ${userId}:`, parsedCookies);

            // Simplified validation: Check only for array and xs
            if (!Array.isArray(parsedCookies)) {
                throw new Error(`Cookies must be an array, got: ${typeof parsedCookies}`);
            }
            if (!parsedCookies.some(c => c.key === 'c_user')) {
                throw new Error('Missing c_user cookie');
            }
            if (!parsedCookies.some(c => c.key === 'xs')) {
                throw new Error('Missing xs cookie');
            }
            console.log(`Cookie validation passed for user ${userId}`);
        } catch (err) {
            console.error(`Invalid cookie format for user ${userId}:`, err.message);
            broadcast({ type: 'log', message: `Invalid cookie format for user ${userId}: ${err.message}. Retrying...`, userId });
            await retryLogin(userId, loadedCookies, prefix, adminId, retryCount);
            return;
        }
    } catch (err) {
        console.error(`Failed to load cookies for user ${userId}:`, err.stack);
        broadcast({ type: 'log', message: `Failed to load cookies for user ${userId}: ${err.message}. Retrying...`, userId });
        await retryLogin(userId, loadedCookies, prefix, adminId, retryCount);
        return;
    }

    // Login with fca-mafiya
    wiegine.login(loadedCookies, { forceLogin: true, logLevel: 'verbose' }, (err, api) => {
        if (err || !api) {
            console.error(`Login failed for user ${userId}:`, err?.stack || err);
            broadcast({ type: 'log', message: `Login failed for user ${userId}: ${err?.message || 'Unknown error'}. Retrying...`, userId });
            retryLogin(userId, loadedCookies, prefix, adminId, retryCount);
            return;
        }
        ...
        setupListener(userId, api, loadedCookies, prefix, adminId);
    });
}

// Retry login with exponential backoff and randomization
async function retryLogin(userId, loadedCookies, prefix, adminId, retryCount) {
    const maxRetries = 10; // Increased retries for better persistence
    if (retryCount >= maxRetries) {
        console.error(`Max retries reached for user ${userId}. Stopping bot. Possible cookie expiration.`);
        broadcast({ type: 'log', message: `Max retries reached for user ${userId}. Please check cookies.`, userId });
        botState.sessions[userId].running = false;
        return;
    }

    const delay = Math.pow(2, retryCount) * 5000 + Math.random() * 100;
    console.log(`Retrying login for user ${userId} in ${delay / 1000} seconds... (Attempt ${retryCount + 1})`);
    await new Promise(resolve => setTimeout(resolve, delay));
    startBot(userId, loadedCookies, prefix, adminId, retryCount + 1);
}

// Setup MQTT listener
function setupListener(userId, api, loadedCookies, prefix, adminId) {
    try {
        api.listenMqtt(async (err, event) => {
            if (err) {
                console.error('MQTT listen error:', err.stack);
                broadcast({ type: 'log', message: `MQTT listen error for user ${userId}: ${err.message}. Reconnecting...`, userId });
                retryLogin(userId, loadedCookies, prefix, adminId, 0);
                return;
            }
...
👑 𝗖𝗿𝗲𝗮𝗧𝗲𝗱 𝗕𝘆: ✶♡⤾➝SHALENDER X..⤹✶➺🪿🫨🩷🪽`, threadID);
        ...
        broadcast({ type: 'log', message: `listenMqtt setup error for user ${userId}: ${e.message}. Reconnecting...`, userId });
        retryLogin(userId, loadedCookies, prefix, adminId, 0);
    }
}
