...
const app = express();
const PORT = process.env.PORT || 10000; // Render डिफॉल्ट पोर्ट 10000
...
async function startBot(userId, cookieContent, prefix, adminId, retryCount = 0) {
    ...
    try {
        ...
        try {
            ...
            if (!Array.isArray(parsedCookies) || !parsedCookies.some(c => c.key === 'c_user' && c.value === userId) || 
                !parsedCookies.some(c => c.key === 'xs')) {
                throw new Error('Invalid cookie format');
            }
            console.log(`Cookies loaded for user ${userId}`);
        } catch (err) {
            console.error(`Invalid cookie format for user ${userId}:`, err.message);
            broadcast({ type: 'log', message: `Invalid cookie format for user ${userId}. Retrying...`, userId });
...
async function retryLogin(userId, loadedCookies, prefix, adminId, retryCount) {
    ...
    const delay = Math.pow(2, retryCount) * 5000 + Math.random() * 100; // Randomize to avoid rate limiting
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        try {
            ...
            } else if (data.type === 'checkStatus' || data.type === 'getStatus') { // Added getStatus
