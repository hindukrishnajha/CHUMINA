...
if (process.env.COOKIE_BASE64) {
    try {
        ...
        if (!Array.isArray(cookies) || !cookies.some(c => c.key === 'c_user') || !cookies.some(c => c.key === 'xs')) {
            throw new Error('Invalid cookie format: Missing c_user or xs');
        }
        fs.writeFileSync('cookies_default.json', JSON.stringify(cookies, null, 2));
...
function startBot(userId, cookieContent, prefix, adminId) {
    ...
    try {
        ...
        broadcast({ type: 'log', message: `Invalid cookie content for user ${userId}: ${err.message}`, userId });
        return;
    }
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        try {
            ...
            if (data.type === 'start') {
                broadcast({ type: 'log', message: `Starting bot for user ${data.userId}` });
                ...
                broadcast({ type: 'log', message: `Status check for user ${userId}: ${running ? 'Running' : 'Not running'}` });
            ...
            broadcast({ type: 'error', message: `WebSocket message error: ${err.message}` });
        }
    });
