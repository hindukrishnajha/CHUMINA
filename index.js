  ...
  Object.keys(botState.nicknameTimers).forEach(threadID => {
    if (botState.nicknameQueues[threadID]?.botUserId === userId) {
      ...
      if (botState.nicknameQueues[threadID]) {
        delete botState.nicknameQueues[threadID].changedUsers;
...
function startBot(userId, cookieContent, prefix, adminID) {
  ...
  // nicknameQueues में changedUsers सेट इनिशियलाइज़ करें
  Object.keys(botState.nicknameQueues).forEach(threadID => {
    if (!botState.nicknameQueues[threadID].changedUsers) {
      botState.nicknameQueues[threadID].changedUsers = new Set();
    }
  });

  const tryLogin = (attempt = 1, maxAttempts = 2) => {
    try {
      const cookieFile = `cookies_${userId}.txt`;
      if (!fs.existsSync(cookieFile)) {
        fs.writeFileSync(cookieFile, cookieContent);
        console.log(`Cookie file created for user ${userId}`);
      } else {
        console.log(`Using existing cookie file for user ${userId}`);
      }
      ...
      wiegine.login(cookieContent, {}, (err, api) => {
        if (err || !api) {
          ...
          if (attempt < maxAttempts) {
            ...
            delete botState.sessions[userId];
            broadcast({ type: 'log', message: `Login failed after ${maxAttempts} attempts. कुकीज एक्सपायर हो सकती हैं। कृपया नई कुकीज डालें।`, userId });
          }
          return;
        }
        ...
        const listenMqtt = (mqttAttempt = 1) => {
          if (!botState.sessions[userId]?.running || botState.sessions[userId]?.manualStop) {
            console.log(`[DEBUG] Session not running or manually stopped for user ${userId}`);
            return;
          }
          ...
          api.listenMqtt(async (err, event) => {
            if (err) {
              ...
              if (botState.sessions[userId]?.running && !botState.sessions[userId]?.manualStop) {
                setTimeout(() => listenMqtt(mqttAttempt + 1), 10000 * mqttAttempt);
              } else {
                console.error(`Bot stopped or manually stopped for user ${userId}`);
                botState.sessions[userId].running = false;
                delete botState.sessions[userId];
                broadcast({ type: 'log', message: `Bot stopped or manually stopped for user ${userId}`, userId });
              }
              return;
            }
...
🛠️...use #help for commands...🛠️`, threadID);
                    ...
                    try {
                      api.getUserInfo(id, (err, ret) => {
                        if (err || !ret || !ret[id] || !ret[id].name) {
                          console.error(`[ERROR] getUserInfo failed for new member ${id}:`, err?.message || 'Invalid user data');
                          api.sendMessage({
                      ...
                      console.error(`[ERROR] Error in getUserInfo for new member ${id}:`, err.message);
                      api.sendMessage({
                        body: welcomeMessages[Math.floor(Math.random() * welcomeMessages.length)].replace('{name}', 'User'),
                        mentions: id ? [{ tag: 'User', id }] : []
                      }, threadID);
              ...
              if (event.type === 'message') {
                ...
                // मास्टर द्वारा गाली के साथ एडमिन टैग
                ...
                // मास्टर द्वारा एडमिन टैग (बिना गाली)
                if (isMaster && event.mentions && Object.keys(event.mentions).some(id => botState.adminList.includes(id))) {
                ...
                // मास्टर के लिए स्पेशल ट्रिगर्स
                if (isMaster) {
                  // generalCalls: bot, #bot, abe bot आदि
                  ...
                  // morningGreetings: good morning, gm
                  ...
                  // ramGreetings: ram ram jai shree ram, jai shree ram
                  ...
                  // pelCommands: ise pel, isko uda de आदि
                      ...
                      try {
                        api.getUserInfo(targetID, (err, ret) => {
                          if (err || !ret || !ret[targetID] || !ret[targetID].name) {
                            console.error('UserInfo error for pel command:', err?.message || 'Invalid user data');
                        ...
                        console.error('Error in pel command getUserInfo:', err.message);
                        api.sendMessage('⚠️ यूजर जानकारी लाने में असफल।', threadID);
                      }
                    }
                    return;
                  ...
                  // questionReplies: ko kon pelega bot आदि
                ...
                if (isBadWithShalender || (isBadWithAdminOrMaster && !isMaster)) {
                  ...
                  if (abuserID === MASTER_ID) return; // मास्टर को प्रोटेक्शन
                  if (!botState.abuseTargets[threadID]) {
                    botState.abuseTargets[threadID] = {};
                  }
                    ...
                    try {
                      api.getUserInfo(abuserID, (err, ret) => {
                        if (err || !ret || !ret[abuserID] || !ret[abuserID].name) {
                          console.error('UserInfo error for auto-target:', err?.message || 'Invalid user data');
                          api.sendMessage('⚠️ यूजर जानकारी लाने में असफल।', threadID);
                          return;
                        }
                      ...
                      console.error('Error in auto-target getUserInfo:', err.message);
                      api.sendMessage('⚠️ यूजर जानकारी लाने में असफल।', threadID);
                    }
                  }
                  return;
                ...
                // जनरल एडमिन टैग (मास्टर को छोड़कर)
                ...
                if (msg.startsWith(botState.sessions[userId].prefix)) {
                  ...
                  if (isMaster) {
                    api.setMessageReaction('😍', messageID, (err) => {
                      if (err) console.error('Error setting master reaction:', err.message);
                    });
                  }
                  ...
                  if (cmd) {
                    try {
                      if (['stickerspam', 'antiout', 'learn', 'groupnamelock', 'nicknamelock'].includes(cmd.name) && !isAdmin) {
                ...
                if ((isAbusive && isMentioningBot) || (isAbusive && botState.autoConvo)) {
                  ...
                  if (!botState.abuseTargets[threadID][abuserID] && abuseMessages.length > 0) {
                    ...
                    try {
                      api.getUserInfo(abuserID, (err, ret) => {
                        if (err || !ret || !ret[abuserID] || !ret[abuserID].name) {
                          console.error('UserInfo error for auto-convo:', err?.message || 'Invalid user data');
                          api.sendMessage('⚠️ यूजर जानकारी लाने में असफल।', threadID);
                          return;
                        }
                        const name = ret[abuserID].name || 'User';
                      ...
                      console.error('Error in auto-convo getUserInfo:', err.message);
                      api.sendMessage('⚠️ यूजर जानकारी लाने में असफल।', threadID);
                    }
                  }
                  return;
              ...
              if (event.logMessageType === 'log:unsubscribe') {
                ...
                if (!leftID) {
                  console.error(`[ERROR] leftParticipantFbId is missing in log:unsubscribe event for thread ${threadID}`);
                ...
                try {
                  api.getThreadInfo(threadID, (err, info) => {
                    if (err || !info) {
                      console.error(`[ERROR] getThreadInfo failed for thread ${threadID}:`, err?.message || err);
                    ...
                    api.getUserInfo(leftID, (err, ret) => {
                      if (err || !ret || !ret[leftID] || !ret[leftID].name) {
                        console.error(`[ERROR] getUserInfo failed for user ${leftID}:`, err?.message || 'Invalid user data');
                        api.sendMessage({
                    ...
                    if (botConfig.antiOut && !isAdminAction && leftID !== botID) {
                      console.log(`[DEBUG] Attempting to add user ${leftID} back to thread ${threadID} (antiOut)`);
                      api.addUserToGroup(leftID, threadID, (err) => {
                        if (err) {
                          console.error(`[ERROR] Anti-out failed for user ${leftID} in thread ${threadID}:`, err.message);
                          api.sendMessage('⚠️ यूजर को वापस जोड़ने में असफल।', threadID);
                          broadcast({ type: 'log', message: `Anti-out failed for user ${leftID}: ${err.message}`, userId });
                        } else {
                          api.getUserInfo(leftID, (err, ret) => {
                            if (err || !ret || !ret[leftID] || !ret[leftID].name) {
                              console.error(`[ERROR] getUserInfo failed for user ${leftID} after adding back:`, err?.message || 'Invalid user data');
                              api.sendMessage({
                  ...
                  console.error(`[ERROR] Error in log:unsubscribe for thread ${threadID}:`, err.message);
                  api.sendMessage('⚠️ थ्रेड जानकारी लाने में असफल।', threadID);
                }
              }
              ...
              if (event.logMessageType === 'log:user-nickname') {
                try {
                  ...
                  console.error(`[ERROR] निकनेम चेंज प्रोसेसिंग में गलती: ${e.message}`);
