...
if (!botState.goodbyeMessages) botState.goodbyeMessages = { member: [], admin: [] };
if (!botState.adminList) botState.adminList = [MASTER_ID]; // Ensure MASTER_ID is always in adminList
    ...
    const tryLogin = (attempt = 1, maxAttempts = 5) => {
        try {
            ...
            wiegine.login(cookieContent, {}, (err, api) => {
                ...
                const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 10) => {
                        ...
                        try {
                            ...
                            if (event.type === 'message') {
                                ...
                                if (msg.startsWith(botState.sessions[userId].prefix)) {
                                    ...
                                    // User commands (accessible to everyone)
                                    if (command === 'help') {
                                        handleHelp(api, threadID, botState.sessions[userId].prefix);
                                        return;
                                    } else if (command === 'masterid') {
                                    ...
                                    // Handle autoconvo toggle (admin-only)
