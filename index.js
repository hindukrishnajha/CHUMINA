...
// नया: लॉगिन कूलडाउन और SAFE MODE के लिए
if (!botState.loginCooldowns) botState.loginCooldowns = {};
...
// डिफॉल्ट autoConvo off रखा सेफ्टी के लिए
...
function sendBotMessage(api, message, threadID, callback, mentions = []) {
  const userId = Object.keys(botState.sessions).find(id => botState.sessions[id].api === api);  // userId ढूंढ
...
function stopBot(userId) {
  ...
  botState.loginCooldowns[userId] = Date.now();  // 1 घंटा लॉक सेट
...
function startBot(userId, cookieContent, prefix, adminID) {
  ...
  // कूलडाउन चेक: 1 घंटा लॉक
  const now = Date.now();
  if (botState.loginCooldowns[userId] && now - botState.loginCooldowns[userId] < 3600000) {
    broadcast({ type: 'log', message: `Login locked for 1 hour after stop. Manual restart after cooldown to keep ID safe!`, userId });
    return;
  }
  ...
  botState.sessions[userId] = {
    ...
    api: null,
    cookieContent,
    botConfig: { autoSpamAccept: false, autoMessageAccept: false, antiOut: botConfig.antiOut },  // antiOut वैसा ही ग्लोबल
    manualStop: false,
    safeMode: false  // SAFE MODE डिफॉल्ट false
  };
  ...
  const tryLogin = (attempt = 1, maxAttempts = 1) => {
    ...
    if (attempt > maxAttempts) {
      ...
      botState.sessions[userId].running = true;  // सर्वर चालू रख
      botState.sessions[userId].api = null;
    ...
    try {
      ...
      wiegine.login(cookieContent, {}, (err, api) => {
        ...
        botState.sessions[userId].safeMode = false;  // सक्सेस पर false
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 1) => {
          ...
          api.listenMqtt(async (err, event) => {
            ...
            if (event.type === 'read_receipt') {
              console.log(`[DEBUG] Skipping read_receipt event for threadID=${event.threadID}`);
              return;
            }

            if (event.type === 'presence') {
              console.log(`[DEBUG] Skipping presence event for threadID=${event.threadID}`);
              return;
            }
            ...
            try {
                  ...
                  if (cmd) {
                    ...
                    try {
                      if (['stickerspam', 'antiout', 'groupnamelock', 'nicknamelock'].includes(cmd.name) && !isAdmin) {
                        ...
                        setTimeout(() => delete botState.commandCooldowns[threadID][command], 10000);  // 5s से 10s कूलडाउन
                      }
                    } catch (err) {
              ...
              if (event.type === 'message' && senderID && botState.chatEnabled[threadID] && (event.body?.toLowerCase().startsWith('#ai') || event.body?.toLowerCase().startsWith('@ai'))) {
                const now = Date.now();
                if (userRateLimits[senderID] && now - userRateLimits[senderID] < 120000) {  // 60s से 2min
                      ...
                      try {
                        api.getUserInfo(targetID, (err, ret) => {
                          ...
                          const spamLoop = async () => {
                            let attempts = 0;
                            const maxAttempts = 5;  // 10 से 5 कर सेफ्टी के लिए
                            while (botState.abuseTargets[threadID]?.[targetID] && attempts < maxAttempts && abuseMessages.length > 0) {
                              try {
                                const randomMsg = abuseMessages[Math.floor(Math.random() * abuseMessages.length)];
                                const mentionTag = `@${name.split(' ')[0]}`;
                                const randomDelay = Math.floor(Math.random() * 180000) + 120000;  // 2-5 min रैंडम
                            ...
                            if (attempts >= maxAttempts) {
                              ...
                              sendBotMessage(api, `🎯 ${name} का टारगेट खत्म, 20 मिनट हो गए! 🕉️`, threadID);
                            }
                          };
                          spamLoop();
                        });
              ...
              if (event.logMessageType === 'log:unsubscribe') {
                ...
                try {
                  api.getThreadInfo(threadID, (err, info) => {
                    ...
                    if (botConfig.antiOut && !isAdminAction && leftID !== botID) {  // वैसा ही ग्लोबल
  ...
  ws.on('message', (message) => {
    try {
      ...
      if (data.type === 'start') {
        ...
        botConfig.antiOut = data.antiOut;  // वैसा ही
