...
if (!botState.deleteNotifyEnabled) botState.deleteNotifyEnabled = {}; // Persist delete notifications state
...
try {
  if (fs.existsSync(LEARNED_RESPONSES_PATH)) {
    ...
    botState.deleteNotifyEnabled = botState.learnedResponses.deleteNotifyEnabled || {}; // Load persisted state
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 3) => {
          ...
          api.listenMqtt(async (err, event) => {
            ...
            try {
              ...
              if (!senderID && !['log:thread-name', 'log:thread-admins', 'typ', 'message_unsend'].includes(event.type)) {  // Yaha 'message_unsend' add kiya
              ...
              if (event.type === 'message') {
                ...
                if (msg.toLowerCase().startsWith('#delete') && (isMaster || Array.isArray(botState.adminList) && botState.adminList.includes(senderID))) {
