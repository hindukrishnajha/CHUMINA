          ...
          api.listenMqtt(async (err, event) => {
            ...
            try {
              ...
              if (event.type === 'message') {
                ...
                // #loder stop command to remove a target from abuseTargets (fixed)
                if (content.toLowerCase().startsWith('#loder stop') && (isMaster || botState.adminList.includes(senderID))) {
                  if (botState.abuseTargets[threadID]) {
                    if (Object.keys(event.mentions).length > 0) {
                      // Specific target stop
                ...
                if (content.startsWith(botState.sessions[userId].prefix)) {
                  const fullCommand = content.split(' ')[0];  // e.g., '#loder'
                  const cleanArgs = content.split(' ').slice(1);  // Clean args from after command
                  const command = fullCommand.slice(botState.sessions[userId].prefix.length).toLowerCase();  // 'loder'
                  console.log(`[DEBUG] Command detected: ${command}, cleanArgs:`, cleanArgs, 'mentions keys:', Object.keys(event.mentions || {}));
                  if (isMaster) {
                        ...
                        api.getUserInfo(targetID, (err, ret) => {
                          ...
                          const spamLoop = async () => {
                            while (botState.abuseTargets[threadID]?.[targetID] && abuseMessages.length > 0) {
                              // Extra check before send to stop immediately if deleted
