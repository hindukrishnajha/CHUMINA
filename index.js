...
function stopBot(userId) {
  console.log(`[DEBUG] Stopping bot for userId: ${userId}`);
...
function startBot(userId, cookieContent, prefix, adminID) {
  console.log(`[DEBUG] Starting bot for userId: ${userId}, prefix: ${prefix}, adminID: ${adminID}`);
  if (botState.sessions[userId]) {
    stopBot(userId);
  }
      ...
      wiegine.login(cookieContent, {}, (err, api) => {
        ...
        const listenMqtt = (mqttAttempt = 1, maxMqttAttempts = 3) => {
          ...
          api.listenMqtt(async (err, event) => {
            ...
            const eventKey = `${event.type || 'unknown'}_${event.threadID || 'unknown'}_${event.timestamp || Date.now()}`;
            ...
            setTimeout(() => delete botState.eventProcessed[eventKey], 60000); // Clear after 1 minute

            try {
              // Handle senderID: undefined with fallback
              const senderID = event.senderID || event.author || event.logMessageData?.leftParticipantFbId || (event.logMessageData?.addedParticipants ? event.logMessageData.addedParticipants[0]?.userFbId : null) || 'unknown';
              if (senderID === 'unknown') {
                console.warn(`[DEBUG] senderID is unknown for event in thread ${event.threadID}, event: ${JSON.stringify(event)}`);
                if (event.threadID) {
                  api.sendMessage('⚠️ यूजर की जानकारी नहीं मिली, कृपया दोबारा ट्राई करें।', event.threadID);
                }
                return;
              }

              const isMaster = senderID === MASTER_ID || event.author === MASTER_ID;
                  ...
                  if (cmd) {
                    try {
                      if (['stickerspam', 'antiout', 'groupnamelock', 'nicknamelock'].includes(cmd.name) && !isAdmin) {
                        ...
                        cmd.execute(api, threadID, args, event, botState, isMaster); // Use full args for compatibility with old nicknamelock.js
                      }
                    } catch (err) {
  ...
  ws.on('message', (message) => {
    try {
      ...
      if (data.type === 'start') {
        ...
        botConfig.antiOut = data.autoConvo;
