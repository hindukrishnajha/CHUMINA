module.exports = {
  ...
  execute(api, threadID, args, event, botState, isMaster) {
    ...
    if (command === 'on') {
      if (args[1].startsWith('@') && args.length >= 3) {
        ...
        // Extract nickname by joining arguments after @user
        const nickname = args.slice(2).join(' ').trim();
        if (!nickname) {
          api.sendMessage('⚠️ कृपया एक निकनेम प्रोवाइड करें।', threadID);
          return;
        }
      ...
      if (args[1] && args[1].startsWith('@')) {
        ...
        api.getUserInfo(userID, (err, ret) => {
          ...
          // Optional: Remove nickname to revert to default
