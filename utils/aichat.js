...
async function getAIResponse(message, isRoast = false) {
  ...
  try {
    ...
    const prompt = isRoast ? ROAST_PROMPT.replace('{message}', truncatedMessage) : `Reply to this message in Hindi, desi style, short and witty, Shalender Hindu ke style mein, without repeating the user's message: ${truncatedMessage || 'अरे भाई, कुछ मस्ती करो ना! 😎'}`;
    
    const chatCompletion = await groq.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: isRoast ? 'You are Shalender Hindu ka roast bot. Generate unique, creative roasts following the roast prompt strictly. Do not repeat user message.' : 'आप एक दोस्ताना AI हैं जो हिंदी में मजेदार और साधारण तरीके से जवाब देता है। जवाब छोटा, मस्ती भरा, और Shalender Hindu के स्टाइल में हो। User का message repeat न करें।'
        },
        {
          role: 'user',
          content: prompt
